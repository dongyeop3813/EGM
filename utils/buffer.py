...
class ReplayBuffer:
    ...
    def add(self, samples, log_r):
        ...
        if self.prioritized == "rank":
            self.scores_np = (
                self.reward_dataset.get_tsrs().detach().cpu().view(-1).numpy()
            )
            ...
            weights = torch.ones_like(self.reward_dataset.get_tsrs())
            self.scores_np = (
                self.reward_dataset.get_tsrs().detach().cpu().view(-1).numpy()
            )
            self.dataset = ZipDataset(self.sample_dataset, self.reward_dataset)
    ...
    def get_data(self):
        """
        Return the all data sample in the buffer.
        """
        return self.sample_dataset.get_seq()
...
def make_buffer(cfg, energy):

    buffer = ReplayBuffer(
        buffer_size=10000,
        device=cfg.device,
        log_reward=energy.energy,
        batch_size=cfg.batch_size,
        data_ndim=energy.data_ndim,
        beta=1.0,
        rank_weight=1e-2,
        prioritized=cfg.prioritized_buffer,
    )

    if cfg.use_gt_sample:
        x1 = energy.sample(20000)
